...
flow_params = dict(
    ...
    exp_tag="worker_scaling_apex_final",
...
config0["num_gpus"] = 1
config0["train_batch_size"] = 128  # batch size
config0["sample_batch_size"] = 50  # batch size
...
config5 = config0.copy()
config5["num_workers"] = 5
configs.append(config5)
config10 = config0.copy()
config10["num_workers"] = 10
configs.append(config10)
config20 = config0.copy()
config20["num_workers"] = 20
configs.append(config20)
config30 = config0.copy()
config30["num_workers"] = 30
configs.append(config30)
config40 = config0.copy()
config40["num_workers"] = 40
configs.append(config40)
config50 = config0.copy()
config50["num_workers"] = 50
configs.append(config50)
config60 = config0.copy()
config60["num_workers"] = 60
configs.append(config60)
config70 = config0.copy()
config70["num_workers"] = 70
configs.append(config70)

for i in range(5):
    for config in configs:
        exp = Experiment("{}_num_workers={}".format(flow_params["exp_tag"], config["num_workers"]), **{
                "run": alg_run,
                "config": {
                    **config
                },
